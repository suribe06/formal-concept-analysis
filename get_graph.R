...
stop_words_es <- tibble(word = unlist(c(read.table("~/Research Articles/Concept Theory + NLP/stop_words_spanish_2.txt", quote="\"", comment.char=""))), lexicon = "custom")
text <- readLines("~/Research Articles/Concept Theory + NLP/article_0.txt")
...
#graficar nube de palabras del articulo
...
#Sentiment Analysis
positive_words <- read.csv("~/Research Articles/Concept Theory + NLP/positive_words_es_2.txt", sep="") %>%
 mutate(sentiment = "Positivo")
colnames(positive_words)[1] <- "word"
negative_words <- read.csv("~/Research Articles/Concept Theory + NLP/negative_words_es_2.txt", sep="") %>%
...
#Graficar nube de palabras por sentimiento
par(mfrow = c(1,1), mar = c(1,1,1,1), mgp = c(0,0,0))
set.seed(123)
text_df %>%
 inner_join(sentiment_words) %>%
...
g <- text_df_skip_counts %>%
  ...
  graph_from_data_frame(directed = FALSE)
g <- igraph::simplify(g)
write.graph(g, "graph_skipgrams.gml", format = "gml")
V(g)$cluster <- clusters(graph = g)$membership
gcc <- induced_subgraph(graph = g, vids = which(V(g)$cluster == which.max(clusters(graph = g)$csize)))
set.seed(123)
